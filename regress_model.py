...
class RegressModel():
    def __init__(self,
        ...
        self.state = tf.compat.v1.placeholder(tf.float32, (None, n_state))
        self.actions = self._build_model(self.state)
        self.target_positions = tf.compat.v1.placeholder(tf.float32, (None, 2))
        self.pos_loss = self._position_loss(self.actions, self.target_positions)
        self.train = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(self.pos_loss)
    ...
    def _position_loss(self, action, targets):
        ...
        x1 = self.env.point0.x + self.env.l0 * tf.math.cos(theta0)
        y1 = self.env.point0.y - self.env.l0 * tf.math.sin(theta0)

        x2 = x1 + self.env.l1 * tf.math.cos(theta1 + theta0)
        y2 = y1 - self.env.l1 * tf.math.sin(theta1 + theta0)

        x3 = x2 + self.env.l2 * tf.math.cos(theta2 + theta1 + theta0)
        y3 = y2 - self.env.l2 * tf.math.sin(theta2 + theta1 + theta0)

        x3 /= self.env.image_size[0]
        y3 /= self.env.image_size[1]

        pos = tf.stack([x3, y3], axis=-1)
        position_loss = tf.compat.v1.losses.mean_squared_error(targets, pos)
